...
describe('Model', function(){
  ...
  describe('Skip 1', function(){
    it('should contain two locations: SD648280 & SD840217', function(done){
      testLocationGet('/locations.json/?skip=1', function(response){
	var res=JSON.parse(response);
	res.should.have.length(2);
	res[0].ref.should.equal(2);
	res[1].ref.should.equal(3);
	res[0].gridRef.should.equal('SD648280');
	res[1].gridRef.should.equal('SD840217');
  ...
  describe('Limit 2', function(){
    it('should contain two locations: SD647277 & SD648280', function(done){
      testLocationGet('/locations.json/?limit=2', function(response){
	var res=JSON.parse(response);
	res.should.have.length(2);
	res[0].ref.should.equal(1);
	res[1].ref.should.equal(2);
  ...
  describe('Skip 1, Limit 1', function(){
    it('should contain one location: SD648280', function(done){
      testLocationGet('/locations.json/?skip=1&limit=1', function(response){
	var res=JSON.parse(response);
